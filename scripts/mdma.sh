...
# to be run (estaticly) by jenkins ci
    ...
    MONARCHIVE = $(MONARCHIVE:-'monarch-archive:/var/www/data/$YYYYMM')
    ...
    scp translation_table/GLOBAL_terms.yaml $MONARCHIVE/translationtable/
    scp rdf_$YYYYMM.tgz $MONARCHIVE/
    scp -r rdf_$YYYYMM.tgz rdf/* $MONARCHIVE/rdf/

    # need a higher resolution than year-month in this case
    git tag -a $(date +%Y%m%d) -m "release candidate $(date +%Y%m%d)"
