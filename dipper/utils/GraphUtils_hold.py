...
class GraphUtils:
    ...
    def addClassToGraph(self, g, node_id, label, type=None, description=None):
        """
        ...
        *if a description is provided,
        ...
        n = self.getNode(node_id)

        g.add((n, RDF['type'], self.OWLCLASS))
        if label is not None:
            g.add((n, RDFS['label'], Literal(label)))
        # label more bnodes
        elif '_:' == node_id[0:2]:                               # TODO: BNODE
            g.add((n, RDFS['label'], Literal(node_id[2:])))
        if type is not None:
    ...
    def addIndividualToGraph(
            self, g, node_id, label, type=None, description=None):
        n = self.getNode(node_id)
    ...
    def addOWLPropertyClassRestriction(
        ...
        node_id = '_:' + property_id + property_value      # TODO: BNODE
        n = self.getNode(node_id)
        g.add((n, RDFS['label'], Literal(node_id[2:])))
    ...
    def addPerson(self, graph, person_id, person_label):
        n = self.getNode(person_id)
        graph.add((n, RDF['type'], self.PERSON))
        if person_label is not None:
            graph.add((n, RDFS['label'], Literal(person_label)))
        #  TODO: BNODE
        elif '_:' == person_id[0:2]:
            graph.add((n, RDFS['label'], Literal(person_id[2:])))
        return
    ...
    def addDefinition(self, g, cid, definition):
        if definition is not None:
            ...
            if '_:' == cid[0:2]:                                 # TODO: BNODE
                g.add((n, RDFS['label'], Literal(cid[2:])))
    ...
    def addDepiction(self, g, subject_id, image_url):
        n = self.getNode(subject_id)
        g.add((n, FOAF['depiction'], Literal(image_url)))
        # TODO: BNODE
        if '_:' == subject_id[0:2]:
            g.add((n, RDFS['label'], Literal(subject_id[2:])))
        return

    def addComment(self, g, subject_id, comment):
        n = self.getNode(subject_id)
        g.add((n, DC['comment'], Literal(comment.strip())))
        # TODO: BNODE
        if '_:' == subject_id[0:2]:
            g.add((n, RDFS['label'], Literal(subject_id[2:])))
        return
    ...
    def addDescription(self, g, subject_id, description):
        n = self.getNode(subject_id)
        g.add(n, DC['description'], Literal(description.strip()))
        # TODO: BNODE
        if '_:' == subject_id[0:2]:
            g.add((n, RDFS['label'], Literal(subject_id[2:])))
        return
    ...
    def addPage(self, g, subject_id, page_url):
        n = self.getNode(subject_id)
        g.add((n, FOAF['page'], Literal(page_url)))
        # TODO: BNODE
        if '_:' == subject_id[0:2]:
            g.add((n, RDFS['label'], Literal(subject_id[2:])))
        return
    ...
    def addTitle(self, g, subject_id, title):
        n = self.getNode(subject_id)
        g.add((n, DC['title'], Literal(title)))
        # TODO: BNODE
        if '_:' == subject_id[0:2]:
            g.add((n, RDFS['label'], Literal(subject_id[2:])))
        return
    ...
    def write_raw_triples(self, graph, filename=None):
        """
         a basic graph writer (to stdout) for any of the sources.
         this will write raw triples in rdfxml, unless specified.
         to write turtle, specify format='turtle'
        ...
        if filename is not None:
            ...
            logger.info("Writing raw triples to %s", filename)

        for (s, p, o) in graph:
            output = [s, p, o]

            print(' '.join(output) + ' .', file=OUTPUT)
    ...
    def write_compact_triples(self, graph, filename=None):
        """
    ...
    def _getNode(self, curie, materialize_bnode):
        ...
        If an id starts with an underscore colon, it assigns it to a BNode,
        otherwise it creates it with a standard URIRef.

        This is problematic in that bnodes are not expected to keep their
        given identifiers. te could includ the "identifier as a label,
        or comment, and give a type and discription but overloading the id
        ...
        node = None
        if curie is not None and '_:' == curie[0:2]:  # TODO: BNODE
            node = BNode(curie[2:])
            #
    ...
    def getNode(self, node_id, materialize_bnode=False):

        return self._getNode(node_id, materialize_bnode)
    ...
    def makeLeader(self, graph, node_id):
        ...
        :return:
        """
        if node_id is not None and '_:' != node_id[0:2]:        # TODO: BNODE
            self.addTriple(
