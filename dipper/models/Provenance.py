...
class Provenance:
    ...
    def add_measurement_data(self, assay_id, measurement_value, measurement_unit, significance=None):
        """
        ...
        ms = self.get_score(self.prov_types['measurement datum'], measurement_value, measurement_unit)
    ...
    def add_measurement_data_to_graph(self, graph, measurement_data):
        ...
        for m in measurement_data:
            s = measurement_data.get(m)
            # we assume that the assay has already been added as an Individual with properties elsewhere
            mid = self.get_score_id(s.get('type'), s.get('value'), s.get('unit'))
            self.gu.addIndividualToGraph(graph, mid, None, s.get('type'))
            self.gu.addTriple(graph, mid, self.data_property['has_value'], s.get('value'))
            sig = s.get('significance')
            if sig is not None:
                sid = self.get_score_id(s.get('type'), s.get('value'), s.get('unit'))
