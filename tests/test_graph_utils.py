...
class GraphUtilsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_graph = self.graph_object.parse("tests/resources/graphutils/gu_test_graph.ttl", format="ttl")
        self.test_graph_id = self.test_graph.identifier
        self.test_graph2 = self.graph_object2.parse("tests/resources/graphutils/gu_test_graph2.ttl", format="ttl")
    ...
    def test_count_predicates(self):
        ...
        self.assertEqual(count.get(self.name), 1, "didn't get correct count for 'name'")
        self.assertEqual(count.get(self.enemyOf), 2, "didn't get correct count for 'enemy of'")
        
    def test_compare_graph_predicates(self):
        self.assertTrue(hasattr(self.graph_util.compare_graph_predicates, '__call__'),
                        "compare_graph_predicates isn't a method or doesn't exist")
        compare = self.graph_util.compare_graph_predicates(self.test_graph, self.test_graph2)
        self.assertTrue(
        ...
        self.assertTrue(
            ...
            "testing hit on both graphs, didn't get correct count for 'name' (graph 1)")
        self.assertTrue(
            compare.get(self.name).get(str(self.test_graph2_id)) == 1,
            "testing hit on both graphs, didn't get correct count for 'name' (graph 2)")
